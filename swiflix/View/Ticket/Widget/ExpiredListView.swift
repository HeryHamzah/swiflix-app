...
struct ExpiredListView: View {
    ...
    var body: some View {
        ...
        if ticketListViewModel.isLoadingGetExpiredTickets {
            TicketListShimmerLoadingView()
        } else if !ticketListViewModel.expiredTickets.isEmpty {
            ScrollView {
                
                VStack( spacing: 20) {
                    ForEach(ticketListViewModel.expiredTickets, id: \.self) { ticket in
